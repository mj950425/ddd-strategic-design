...
| 주문 상태      | order status             | 주문의 진행상황을 의미한다.                                             |
| 대기중        | waiting                  | 손님의 주문을 요청했지만, 아직 가게 사장님이 수락하지 않은 상태이다.                     |
| 수락됨        | accepted                 | 가게 사장님이 손님의 주문을 수락한 상태이다.                                   |
| 제공됨        | served                   | 조리가 완료되어 손님에게 음식을 전달한 상태이다.                                 |  
| 완료         | completed                | 손님의 음식을 치운 상태이다.                                            |
| 주문 생성 시간   | order date time          | 주문 데이터가 생성된 시간을 의미한다.                                       |  
| 주문 생성      | create order             | 주문을 생성하는것을 의미한다.                                            |  
| 주문 아이템     | order line item          | 주문 받은 아이템을 의미한다.                                            |  
| 주문 아이템 수량  | order line item quantity | 주문 받은 아이템의 수량을 의미한다.                                        |  
...
- 속성
    - 메뉴는 이름, 가격, 메뉴 그룹, 메뉴 상품 리스트, 노출 여부를 가진다.
    - 메뉴의 이름은 비어있거나 비속어가 포함될 수 없다.
    - 메뉴의 가격은 0원 이상이어야 한다.
    - 메뉴에 포함된 모든 메뉴 상품의 가격과 개수의 곱을 합한 값이 메뉴의 가격보다 작을 수 없다.
    - 메뉴 상품의 개수는 0 이상이어야한다.
- 행위
    - 메뉴를 생성한다.
    - 메뉴 가격을 변경한다.
...
- 속성
    - 주문은 주문 유형, 주문 상태, 주문 시간, 배달 주소, 주문 테이블, 주문 아이템 목록을 가진다.
    - 배달 주문은 주문 유형이 DELIVERY이다.
    - 주문 상태는 WAITING, ACCEPTED, SERVED, DELIVERING, DELIVERED, COMPLETED 중에 하나여야 한다.
    - 주문 유형이 EAT_IN이 아닌 경우 주문 아이템의 수량은 0 이상이어야 한다.
- 행위
    - 주문을 생성한다.
        - 주문 생성 요청의 메뉴는 노출 상태이어야 한다.
        - 주문 생성 요청의 주문 아이템은 비어있을 수 없다.
        - 주문 생성 요청의 배달 주소는 비어있을 수 없다.
        - 주문 생성 요청의 메뉴는 이미 저장된 상태이어야 한다.
        - 주문 생성 요청의 주문 아이템 개수는 0 이상이어야 한다.
        - 주문 생싱 요청의 주문 아이템의 가격은 실제 메뉴 가격과 일치해야 한다.
        - 초기 생성된 주문은 주문 상태가 WAITING이다.
    - 주문을 접수한다.
        - 접수 시점에 주문 상태는 ACCEPTED로 변경된다. 이 때, 기존 주문의 상태는 WAITING이어야 한다.
        - 배달 대행 업체에 배달을 요청한다.
    - 주문한 음식을 제공한다.
...
- 속성
    ...
    - 주문 테이블의 이름은 비어있지 않아야 한다.
- 행위
    - 주문 테이블을 생성한다.
        - 초기에 주문 테이블을 생성하면 점유가 안되어있으며 착석한 손님 수는 0명이다.
    - 주문 테이블에 착석한다.
        - 주문 테이블을 착석하면 점유 상태가 된다.
    - 주문 테이블의 착석한 손님 수를 변경한다.
        - 변경 요청의 손님 수는 0 이상이어야 한다.
        - 변경 요청의 주문 테이블은 점유 상태이어야한다.
    - 주문 테이블을 초기화한다.
        - 주문 테이블을 초기화하면 점유 상태가 해제되고 착석한 손님 수는 0명이 된다.
...
sequenceDiagram
    ...
    rect rgba(200, 200, 255, 0.3)
        ...
        alt 배달 또는 포장 유형의 주문인데, 주문 아이템 개수가 0보다 작음
            주문 -->> 사장님: 예외 발생
        end
        alt 메뉴가 표시되지 않음
            주문 -->> 사장님: 예외 발생
